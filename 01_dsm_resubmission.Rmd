...
params:
  seed_no: 16
  species_name: "duiker"
  segdata_file: "segdata_duiker.xlsx"
  obsdata_file: "obsdata_duiker.xlsx"
  preddata_file: "preddata_duiker_new.xlsx"
  conversion_km_m: !expr 1/200
output:
  html_document:
    df_print: paged
    toc: yes
...
load(paste("output/fitVU_", params$species_name, "_modelcomparison.RData", sep = ""))
...
```{r}
dat <- make.data_mod2(response = "abundance.est", ddfobject = fitVU,
...
dat
```
(please not that the `make.data` function takes a `convert.units` argument, whose value is specified in the header taking into account the conversion from km to m and whose absolute value depends on the Effort too, i.e. it must be set equal to \(1/Effort\)).
...
dsm.var.subset <- dsm_var_gam(dsm.obj=mod1, pred.data=preddata.varprop.subset,
                              ...
                              type.pred = "response")
fName = attr(est$hr,"fName")
summary.dsm.var_mod(object = dsm.var.subset, detfunc = fitVU)
```

## Maps of abundance and uncertainty

Finally, we can plot the results to show the abundances within the study area, and the corresponding uncertainty map. The spatially explicit abundance data are also saved to a .csv file (`r paste("DSMresults_", params$species_name, ".csv")`, in the output folder) for later use in a GIS software, if needed.
...
# uncomment the following two line to save the output again
# write.csv(pp,paste("output/DSMresults_", params$species_name,
#                    "_modelcomparison.csv", sep = ""))
```
