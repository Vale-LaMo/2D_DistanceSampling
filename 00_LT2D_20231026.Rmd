...
**To perform the analyses, initial parameters (species name, input file, percentage used for perpendicular distance data truncation, forward truncation distance in meters, etc.) are set at the beginning of this notebook (they can be customized manually, or via "Knit with parameters" in the Knit menu).**
...
The best models (\(\Delta\)AIC < 2) are identified and plotted (there might be only one). The figure(s) shows the actual distribution of animals (continuous black line), the "observed" detection function (bold red line) and the "corrected" detection function (dashed black line), that takes into account the behavioural response
...
```
Among them, if more than one, we finally select the model with the lowest CV:
...
if(cv_clustersize == 0){
  ind_min <- stats_df_groups$NhatLower
  ind_max <- stats_df_groups$NhatUpper
} else {
  (abund_survey_individuals[[1]]-(abund_survey_individuals[[1]]/cv_tot)*1.96)[[1]] -> ind_min
  (abund_survey_individuals[[1]]+(abund_survey_individuals[[1]]/cv_tot)*1.96)[[1]] -> ind_max
}
