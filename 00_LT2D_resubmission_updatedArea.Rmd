...
```{r output-options, include = FALSE}
knitr::opts_chunk$set(echo=TRUE) # shows/hide code in html output
```
...
source("functions/DS2D_custom_functions.R")
# functions for paper figs
source("functions/plotfit.x.red.ggplot.R")
source("functions/plot.ds.ggplot.R")
```
...
Here, we rely on the functions from LT2D package (https://github.com/david-borchers/LT2D), as partly revised and applied in Elenga et al. (2020) (https://github.com/cbonenfant/duikers-abundance). With respect to the latter, we made minor changes to the code. The code, data and functions used are available on https://github.com/Vale-LaMo/2D_DistanceSampling

**To perform the analyses, initial parameters (species name, input file, percentage used for perpendicular distance data truncation, forward truncation distance in meters, number of models) are set at the beginning of this notebook (they can be customized manually, or via "Knit with parameters" in the Knit menu)**
<!-- - except for the starting values that can only be customized manually, but normally they should not be edited - see below).** -->
...
```{r data-loading}
#### Load dataset
data <- read_excel(paste("data/",params$input_file,sep=""), 
                   col_types = c(rep("numeric", 8),
                                 "date",
                                 "text", "text"))
# the extent of the area is changed to be consistent with the preddata later used in DSM
if (params$species_name == "impala") data$area <- 536 else data$area <- 519
```
...
```{r plot-avoidance, warning=FALSE}
opar <- par(pty="s")
plot(data_clean$perp_dist, data_clean$forw_dist,
     xlim=c(0,max(c(data_clean$perp_dist,data_clean$forw_dist))),
     ylim=c(0,max(c(data_clean$perp_dist,data_clean$forw_dist))),
     xlab = "Perpedincular distance (m)",
...
if(params$species_name == "duiker") {
  p <- ggplot(data_clean, aes(x = perp_dist, y = forw_dist)) +
    geom_point() +  # Add points for the scatter plot
    xlim(0, max(c(data_clean$perp_dist, data_clean$forw_dist))) +
    ylim(0, max(c(data_clean$perp_dist, data_clean$forw_dist))) +
    labs(
    ...
    annotation_custom(rasterGrob(logo, width = unit(2, "cm"), height = unit(2, "cm")), xmin=65, ymin=70)
  # Print the plot
  # print(p)
  p_avoidance_duiker <- p
  # saveRDS(p, file = paste("output/p_avoidance_",params$species_name,".rds",sep=""))
}

# # Paper fig
logo_path <- paste("logos/", params$species_name, "_logo.png",sep = "")
logo <- readPNG(logo_path)
if(params$species_name == "impala") {
  p <- ggplot(data_clean, aes(x = perp_dist, y = forw_dist)) +
    geom_point() +  # Add points for the scatter plot
    xlim(0, max(c(data_clean$perp_dist, data_clean$forw_dist))) +
    ylim(0, max(c(data_clean$perp_dist, data_clean$forw_dist))) +
  ...
  # Print the plot
  # print(p)
  p_avoidance_impala <- p
  # saveRDS(p, file = paste("output/p_avoidance_",params$species_name,".rds",sep=""))
}
```

We now select the truncation distances. For this purpose, we produce histograms and boxplots to identify outliers for the perpendicular distances:
...
**Please check these latter plots and eventually go back to the header to set a truncation distance for forward distances.**.   
...
Following Elenga et al. (2020), we model the detection in two dimensions using two alternative functions: a radial exponential hazard risk (\(h_{HB}\) under the notation of Borchers & Cox 2017), and a radial exponential with longitudinal (*y*) translation for \(h(0)<1\).
The radial exponential hazard risk function \(h_{HB}\) makes the same approximation as the half-normal detection function that is commonly found to describe the detection process in 1D. That is, we use the `h.RE` function of Elenga et al. (2020) for modeling the decay in detection rate with radial distance. For the radial exponential with longitudinal (*y*) translation for \(h(0)<1\), we use the `h.yTRE` function of Elenga et al. (2020), which is useful if the detection probability at distance zero is already imperfect.  
The `h.RE` function can be coupled with the `pi.sigmo` functions of Elenga et al. (2020) for modeling the change in animal density with perpendicular distance to a line-transect (i.e., the behavioural response), or alternatively with the `pi.sigmoI` (sigmoid with Intercept) if the animals do not completely avoid the transect even at distance zero from the observer. The `h.yTRE` function can be coupled only with `pi.sigmo`. See also Elenga et al. (2020) for details, or the files "functions/com_hfunctions.R" and "functions/com_pifunctions.R" for alternative functions.
The choice of the function should actually be performed by the data analyst. However, to ease the application, here we automate this step by fitting both functions, and then selecting the best one based (a) on the results of goodness of fit tests (only models with GOF p > 0.1 are retained), and (b) according to a standard AIC model selection procedure.  
...
`r params$n_models` (number set via the parameter `n_models` in the header) are fitted using parameter values extracted from a random distribution, with the initial values from the mean (`starting_values`) and the sd (`sd`) declared when applied the function. The length of the `starting_values` depends on the number of parameters that characterize the adopted functions.

```{r model-fitting}
hr = h.RE; pi.x = pi.sigmo; fName = attr(hr,"fName")
fit_model_1 <- fit_model(hr, pi.x, starting_values = c(0.25,0.25,-4,-1), sd = 6)
analyze_selected_models(hr, pi.x, fit_model = fit_model_1) -> res_model_1

hr = h.RE; pi.x = pi.sigmoI; fName = attr(hr,"fName")
fit_model_2 <- fit_model(hr, pi.x, starting_values = c(0.25,0.25,-4,-1,-1), sd = 6)
analyze_selected_models(hr, pi.x, fit_model = fit_model_2) -> res_model_2

hr = h.yTRE; pi.x = pi.sigmo; fName = attr(hr,"fName")
fit_model_3 <- fit_model(hr, pi.x, starting_values = c(0.25,0.25,0.25,-4,-1), sd = 6)
analyze_selected_models(hr, pi.x, fit_model = fit_model_3) -> res_model_3
```

```{r best-models}
rbind(res_model_1, res_model_2, res_model_3) %>% 
  arrange(modAICc) %>% 
  # filter(modAIC <= min(modAIC) + 4) %>% 
  # filter(CV.phat == min(CV.phat))
  filter(CV.phat < 0.9) %>% 
  mutate(best_mod_no = seq_along(m)) %>% 
  relocate(best_mod_no, .before = m) -> tabAIC
tabAIC
```

<!-- The models with the parameter values that fulfill the goodness of fit condition (GOF tests \(> 0.1\)), with low AIC values (considering a \(\Delta AIC < 2\)), and a CV\(< 0.90\), are retained and plotted. -->
The models with the parameter values that fulfill the goodness of fit condition (GOF tests \(> 0.1\)) are arranged based on AIC values. As a rule, only models with \(\Delta AIC < 2\) or \(\Delta AIC < 4\) should be considered. However, the model selection procedure should be also based on a visual inspection of the detection functions.
For these reason, we extend the following analyses to a larger, while still reasonable, set of models. Indeed, the following figures show the actual distribution of animals (continuous black line), the "observed" detection function (bold red line) and the "corrected" detection function (dashed black line), that takes into account the behavioral response, for the first 10 models (this value can be changes by modifying the following chunk, if more models need to be considered).

```{r set-no.models}
no.models = 10
if(no.models > dim(tabAIC)[1]) no.models = dim(tabAIC)[1]
```

```{r visual-inspection-detfunc}
data_trunc$perp_dist -> x
w = ceiling(max(x))

# for (i in 1:dim(tabAIC)[1]) {
for (i in 1:no.models) {
  if(tabAIC[i,]$h_func == "h.RE" & tabAIC[i,]$pi_func == "pi.sigmo") models <- fit_model_1$FIT
  if(tabAIC[i,]$h_func == "h.RE" & tabAIC[i,]$pi_func == "pi.sigmoI") models <- fit_model_2$FIT
  if(tabAIC[i,]$h_func == "h.yTRE") models <- fit_model_3$FIT
  plotfit.x.red(x[x<=w],models[[tabAIC[i,]$m]],nclass=20,nint=100,
                sub = paste("best model no. ", i, tabAIC[i,]$h_func, tabAIC[i,]$pi_func, sep = " - "));rug(x[x<=w])
}
```

Based on the model table and the visual inspection of the plots, we finally choose the best model.   

**PLEASE BE AWARE THAT THE NUMBER IN THE FIRST LINE OF THE NEXT CHUNK MUST BE ENTERED MANUALLY, BASED ON THE CHOSEN MODEL (e.g., 4 if the chosen model is the fourth one in the AIC table)**

```{r}
best_mod_no <- 3 # insert model number here!
best_mod <- tabAIC[best_mod_no,] 
if(best_mod$h_func == "h.RE") hr = h.RE else hr = h.yTRE
if(best_mod$pi_func == "pi.sigmo") pi.x = pi.sigmo else pi.x = pi.sigmoI
if(best_mod$pi_func == "pi.sigmoI") {
  starting_values = c(0.25,0.25,-4,-1,-1)
} else {
  if(best_mod$h_func == "h.RE") {
    starting_values = c(0.25,0.25,-4,-1)
  } else {
    if(best_mod$h_func == "h.yTRE") {
      starting_values = c(0.25,0.25,0.25,-4,-1)
    }
  }
}
fName = attr(hr,"fName")
fit_best_model <- fit_model(hr, pi.x, starting_values, sd = 6)
fitVU = fit_best_model$FIT[[best_mod$m]]
save(fitVU, file = paste("output/fitVU_", params$species_name, "_modelcomparison.RData", sep = ""), compress = FALSE)
```
...
data_trunc$perp_dist -> x
w = ceiling(max(x))
plotfit.x.red(x[x<=w],fitVU,nclass=20,nint=100, sub = paste("Best model no. ", best_mod_no, sep = ""));rug(x[x<=w])

# # paper fig for duiker
# source("functions/plotfit.x.red.ggplot.R")
if(params$species_name == "duiker")  {
  plotfit.x.red.ggplot(x[x<=w],fitVU,nclass=20,nint=100,
          ymx=0.08,xaxislabel="",
          xminlogo = 30, yminlogo = 0.05) -> p_detection_duiker
}
# saveRDS(detection_plot_duiker, file = "output/detection_plot_duiker.rds")

# 30 0.05

# # paper fig for impala
if(params$species_name == "impala")  {
  plotfit.x.red.ggplot(x[x<=w],fitVU,nclass=20,nint=100,
                       species = "impala",
                       ymx=0.015,
                       xaxislabel="Perpendicular distance (x)",
                       showlegend = FALSE,
                       xminlogo = 75, yminlogo = 0.01) -> p_detection_impala
}
```

For the chosen model, we now show in detail the checks performed to verify the goodness of fit in the perpendicular dimension (Kolmogarov-Smirnov and Cramer-von Mises p-values are also reported):
...
We also show the goodness of fit in the forward dimension

```{r}
# GoF for forward distances
fName = attr(pi.x, "fName")
...
The resulting CDS detection function is shown in the following figure:
```{r cds-plot}
...
if(params$species_name == "duiker") {
  nc <- 5
  breaks <- seq(0,cds.hr$ddf$meta.data$width,cds.hr$ddf$meta.data$width/nc)
  source("functions/plot.ds.ggplot.R")
  ggplot(data = cds_plot_data[[1]], aes(x=mids, y=density)) + 
    geom_bar(stat="identity", group=1, fill="grey", col="black", width = width/nc) +
    geom_line(data=cds_plot_data[[2]],
              aes(x = observed_distances, y = fitted_values), color = "black", size = 1) +
    ...
    guides(x = "axis_truncated", y = "axis_truncated") +
    annotation_custom(rasterGrob(logo, width = unit(2, "cm"), height = unit(2, "cm")), xmin=40, ymin=1.5) -> p
  # Print the plot
  # print(p)
  p_cds_duiker <- p
}
...
if(params$species_name == "impala") {
  ...
  ggplot(data = cds_plot_data[[1]], aes(x=mids, y=density)) + 
    ...
    labs(title = "", 
             ...
             y = "Detection probability") +
    guides(x = "axis_truncated", y = "axis_truncated") +
    annotation_custom(rasterGrob(logo, width = unit(2.5, "cm"), height = unit(2.5, "cm")), xmin=100, ymin=1) -> p
  # Print the plot
  # print(p)
  p_cds_impala <- p
}
...
The results are then compared to those obtained above with the 2D-DS approach:
