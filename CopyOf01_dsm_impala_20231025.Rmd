...
params:
  seed_no: 16
  species_name: "impala"
  segdata_file: "segments400.xlsx"
  obsdata_file: "obsdata_impala.xlsx"
  preddata_file: "grid400_vlm.xlsx"
...
and additional, optional columns can be added, to describe the segments in terms of environmental covariates (e.g., here we imagined to include information on habitats, distance from water sources or from anthropic areas such as villages, altitude, ...).    
All optional fields can have empty cells (or *NA*) in the Excel file (although this will affect their use in the following modelling procedures). On the contrary, *NA*s are not admitted in the fields: `area`, `Transect.Label`, `Sample.Label`, `x`, `y`, `Effort`.   
...
Finally, the `preddata` dataset (Excel file) should include at least the following columns:
- `id`: a unique identifier for each grid cell in the study area
- `x`: x-coord of the center of the cell
- `y`: y-coord of the center of the cell
- `area`: the area of each cell (in m\(^2\))
Please pay attention to the coordinates of the cells, that should be in the same reference system used for the other datasets, in particular the `segdata`.
Additional columns including the values for environmental covariates can also be included.
...
off.set = log dell'area del segmento
...
STOP QUI, L'ANALISI DELLA VARIANZA SI IMPALLA, CERCO POI DI RISOLVERE SUCCESSIVAMENTE
...
summary.dsm.var.vale(dsm.xy.var)

SE = (upper limit – lower limit) / 3.92
SE = (27505.77 - 3232.511) /3.92
SE/9429.353

9429/((dim(preddata))[1]) # densità su 16 ettari, o 0.16 km2
1.4:0.16 = x:1
1.4/0.16 # densità al km2
...
## --- Variance estimation per il modello scelto ----
#
# righe da non modificare
preddata.varprop.subset <- split(preddata, 1:nrow(preddata))
offset.varprop.subset <- as.list(rep(preddata$area[1],nrow(preddata)))
# # solo per paper proprietà private
# preddata.varprop.subset_pp <- split(preddata.subset2_pp, 1:nrow(preddata.subset2_pp))
# offset.varprop.subset_pp <- as.list(rep(off.set,nrow(preddata.subset2_pp)))
#
...
# # solo per paper proprietà private
# dsm.var.subset_pp <- dsm.var.gam(dsm.obj=mod, pred.data=preddata.varprop.subset_pp, 
#                                  off.set = offset.varprop.subset_pp, seglen.varname = "Effort", 
#                                  type.pred = "response")
# summary(dsm.var.subset_pp)
# NB, il CV va moltiplicato per 100
...
# write.csv(pp.uncertainty_pp,"results/2020-07-08-DSMresults2015_grigio_pp.csv") # per paper proprietà private
    ...
    # the delta method
      ...
      for(i in seq_along(ddf)){
        ...
        if(all(class(this_ddf)!="fake_ddf")){
          ddf.summary <- summary(this_ddf) # errore a questo punto, perché è una fake

          this_cvp.sq <- (ddf.summary$average.p.se/
                          ddf.summary$average.p)^2
          cvp.sq <- cvp.sq + this_cvp.sq # mi serve il cv di phat che ho già di là
        }else{
          this_cvp.sq <- NA
        }
...
```{r}
summary.dsm.var_vale <- function(object, alpha=0.05, boxplot.coef=1.5,
